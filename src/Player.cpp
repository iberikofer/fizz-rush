...
Player::Player(float winWidth, float winHeight) : m_player(m_playerTexture), m_playerAura(m_playerAuraTexture1), m_arrow(m_arrowTexture), m_wallSound(m_wallSoundBuffer), m_hitSound(m_hitSoundBuffer), m_rollSound(m_rollSoundBuffer)
{
...
void Player::update(Time dt, float winWidth, float winHeight, float m_machineLeftWall, float m_machineRightWall)
{
	...
	//? Coming soon In 2nd Episode
	// bool m_isCanMoving = false;
	// if (Keyboard::isKeyPressed(Keyboard::Key::Left) || Keyboard::isKeyPressed(Keyboard::Key::Right) || Keyboard::isKeyPressed(Keyboard::Key::Up) || Keyboard::isKeyPressed(Keyboard::Key::Down))
	// {
	// 	m_isCanMoving = true;
	// }
	// if (m_isCanMoving && m_rollSound.getStatus() != Sound::Status::Playing)
	// {
	// 	m_rollSound.play();
	// }
	// else if (!m_isCanMoving)
	// {
	// 	m_rollSound.stop();
	// 	m_isCanMoving = false;
	// }
...
{
	//* === WALLS ===
	FloatRect m_spriteBounds = getHitbox();
...
void Player::resetGame(float m_startPosX, float m_startPosY, int maxHP, float difficultySpeed, float invincibilityDuration)
{
...
void Player::draw(RenderWindow &window, const GameSettings &gameSettings)
{
	...
	{
		FloatRect playerHitbox = getHitbox();
		RectangleShape playerDebugRect({playerHitbox.size.x, playerHitbox.size.y});
		playerDebugRect.setPosition({playerHitbox.position.x, playerHitbox.position.y});
		playerDebugRect.setFillColor(Color::Transparent);
		playerDebugRect.setOutlineColor(Color::Cyan);
		playerDebugRect.setOutlineThickness(2.0f);
		window.draw(playerDebugRect);
	}
}