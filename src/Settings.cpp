    ...
    if (std::getline(iss, key, '=')) {
      ...
      if (std::getline(iss, value)) {
        try {
          ...
          if (key == "difficulty")
            gameDifficulty = static_cast<GameDifficulty>(intValue);
          else if (key == "windowMode")
            windowMode = static_cast<WindowMode>(intValue);
            ...
            showFps = intValue;
          else if (key == "showHitbox")
            showHitbox = intValue;
          else if (key == "ukrainianLanguage")
            ukrainianLanguage = intValue;
        } catch (...) {
...
void GameSettings::saveToFile(const std::string &filename) const {
  ...
  file << "# 1 - ON, 0 - OFF. difficulty: 0 - easy, 1 - medium, 2 - hard\n\n";
