...
public:
  ...
  void update(sf::Time dt, float winWidth, float winHeight,
              float m_machineLeftWall, float m_machineRightWall,
              float m_machineUpWall, float m_WallPushBack, bool playWallSound,
              int currentEpisode);
  void stopSound();
  void checkWorldCollision(float winWidth, float winHeight, float leftWall,
                           float rightWall, float topWall, float m_WallPushBack,
                           bool playWallSound);
  ...
  int loseHealth();
  void resetGame(float m_startPosX, float m_startPosY, int maxHP,
